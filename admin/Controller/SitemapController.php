...
class SitemapController extends AppController {
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'Sitemap.id DESC');
		$sitemap = $this->paginate('Sitemap');
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			if(empty($this->request->data['Sitemap']['parent_id'])) $this->request->data['Sitemap']['parent_id'] = 0;
        ...
        $list_sitemap = $this->get_category('Sitemap');
        $this->set('list_sitemap', $list_sitemap);
    }
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'Sitemap', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data['Sitemap'] = $this->request->data['Sitemap'];
            if($data['Sitemap']['parent_id'] == '') {
                $data['Sitemap']['parent_id'] = 0;
            }
            if ($this->Sitemap->save($data['Sitemap'])){
	...
	*/
	public function copy($id)
	{
		$this->Session->setFlash(__('', true));
		if(!empty($id)){
			$data = $this->Sitemap->read(null, $id);
			$this->data = $data;
            $list_sitemap = $this->get_category('Sitemap');
            $this->set('list_sitemap', $list_sitemap);
			$this->render('add');
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Sitemap->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Sitemap->save(array('Sitemap'=>array('status'=>1, 'id'=>$id)));
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Sitemap->updateAll(array('status'=>1), array('Sitemap.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Sitemap->save(array('Sitemap'=>array('status'=>0, 'id'=>$id)));
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Sitemap->updateAll(array('status'=>0), array('Sitemap.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'Sitemap.name like "%'.$keyword.'%"',
							'Sitemap.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Sitemap.id DESC, Sitemap.modified DESC'
			);
			$sitemap = $this->paginate('Sitemap');
        	$this->set('sitemap', $sitemap);
			$this->render('index');
